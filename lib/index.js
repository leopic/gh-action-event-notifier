...
module.exports.run = async () => {
  ...
  try {
    const jobStatus = process.env.JOB_STATUS || core.getInput('job-status', {required: true});
    const runId = process.env.RUN_ID || core.getInput('run-id', {required: true});
    ...
    message = blockBuilder.getBlockBuilder(eventName, jobStatus, payload, runId);
