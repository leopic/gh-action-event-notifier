...
module.exports.getBlockBuilder = (eventName, jobStatus, payload) => {
  switch (eventName) {
    case 'pull_request':
      this.getPullRequestBlocks(jobStatus, payload);
      break;
    case 'push':
      this.getPushBlocks(jobStatus, payload);
      break;
    default:
      this.getFallbackBlocks(jobStatus, payload);
  }
};
...
module.exports.run = () => {
  try {
    ...
    console.log('___ github.context', github.context);
    // console.log('___ github.context.payload', github.context.payload);

    const blocks = this.getBlockBuilder(github.context.eventName, jobStatus, github.context);
    console.log('___ blocks', blocks);

    const formattedBlocks = this.formatBlocks(blocks);
    console.log('___ formattedBlocks', formattedBlocks);
