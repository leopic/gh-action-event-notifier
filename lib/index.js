...
module.exports.getCommitListMessage = payload => {
  ...
  payload.commits.map(commit => {
    let commitMessage = `- <${commit.url}|${this.getShortSha(commit.id)} ${commit.message}> by ${commit.author.name}\n`;
...
module.exports.getPushBlocks = (jobStatus, payload) => {
  ...
  const secondLine = `A *${verb}* was made to *${target}* ${emoji}.\n`;
  const thirdLine = `Head is now ${sha}\n`;
  const message = [firstLine, secondLine, thirdLine];
